...
export const BinComponent = ({
  ...
  const maxPathHeight = 296.166; // pixel height the path
  const [binPathHeight, setBinPathHeight] = useState(0); // animate path height
  const [yPosition, setYPosition] = useState(maxPathHeight); // animate y position

  useEffect(() => {
    const newBinPathHeight = (binPathHeightPercent / 100) * maxPathHeight; // percentage to pixel height
    setBinPathHeight(newBinPathHeight); // state
    setYPosition(maxPathHeight - newBinPathHeight); // y position state
